...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(model.conversation.type==EMConversationTypeChat)
    {
        //如果是单聊
        //将名称替换为昵称-bing
        UserProfileEntity * entity=[[UserProfileManager sharedInstance]getUserProfileByUsername:model.title];
        model.title= entity.nickname;
        model.avatarURLPath=entity.imageUrl;
    }
    else if(model.conversation.type==EMConversationTypeGroupChat)
    {
        ...
        if(extDic[@"subject"])
        {
            NSString * title= [model.conversation.ext objectForKey:@"subject"];
            //设置会话名称为群组名称
            model.title=title;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(conversationListViewController:didSelectConversationModel:)]) {
        ...
        if(model.conversation.type==0)
        {
            //单聊
            //设置聊天的名称为好友的昵称
            NSString * title= [[UserProfileManager sharedInstance]getNickNameWithUsername:model.title];
