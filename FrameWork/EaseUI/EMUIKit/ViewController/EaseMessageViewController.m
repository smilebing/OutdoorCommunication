...
- (void)_locationMessageCellSelected:(id<IMessageModel>)model
{
    ...
    NSDictionary * extMsg= model.message.ext;
    if(extMsg.count!=0)
    {
        //大头针标题
        NSString * title=[extMsg objectForKey:@"title"];
        //详细信息
        NSString * detail=[extMsg objectForKey:@"detail"];
    }
    
    //显示目的地的位置
    DaoHangMapVC * daohangMap=[[DaoHangMapVC alloc ]initWithLocation:CLLocationCoordinate2DMake(model.latitude, model.longitude) extendMsg:extMsg];
    [self.navigationController pushViewController:daohangMap animated:YES];
