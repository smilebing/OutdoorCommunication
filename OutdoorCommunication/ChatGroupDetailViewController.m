...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.row == 5)
    {
        ...
        cell.detailTextLabel.text= self.chatGroup.description;
    }
    else if (indexPath.row == 6)
    {
        cell.textLabel.text=@"查看群组人员位置";
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.row == 4)
    {
        ...
        //显示群组位置
        GroupUserLocationVC * groupUserLocVC=[[GroupUserLocationVC alloc]init];
        [groupUserLocVC setGroupID:_chatGroup.groupId];
        [self.navigationController pushViewController:groupUserLocVC animated:YES];
...
- (void)dissolveAction
{
    ...
    NSDictionary * paraments=@{@"geotable_id":LBS_geotable_id,
                               @"ak":LBS_ak,
                               @"tags":_chatGroup.groupId                               };
    
    //发送post数据
    [self.session POST:LBS_DELETE_POINT_URL parameters:paraments progress:^(NSProgress * _Nonnull uploadProgress) {
...
//                [self hideHud];
//                //发送数据出错
//                [EMAlertView showAlertWithTitle:@"提示" message:@"群组解散失败" completionBlock:^(NSUInteger buttonIndex, EMAlertView *alertView) {
//                    NSLog(@"删除数据平台群组数据失败%ld",(long)status);
