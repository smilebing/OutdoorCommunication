...
@property (nonatomic,copy)NSString * addressSubTitle;

//判断是否为地理位置的群组
@property (nonatomic)NSInteger  isLocationGroup;
//坐标
...
-(instancetype) initWithDetail: (NSString * )address subTitle:(NSString *) description location:(CLLocationCoordinate2D) groupLocation
{
    ...
    {
        //设置群组为地理位置群组
        self.isLocationGroup=1;
        self.addressTitle=address;
        self.addressSubTitle=description;
...
- (EMTextView *)textView
{
    if (_textView == nil) {
        ...
        if([self.addressTitle isEqualToString:@""]&&[self.addressSubTitle isEqualToString:@""])
        {
            _textView.placeholder = NSLocalizedString(@"group.create.inputDescribe", @"please enter a group description");
        }
        else
        {
            _textView.text=self.addressSubTitle;
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (group && !error) {
                ...
                if(_isLocationGroup==1)
                {
                    ...
                    //将群组的位置信息上传到服务器
                    NSDictionary * paraments=@{@"geotable_id":LBS_geotable_id,
                                               @"coord_type":@3,
                                               @"ak":LBS_ak,
                                               @"longitude":longitude ,
                                               @"latitude":latitude ,
                    ...
                    //发送post数据
                    [self.session POST:LBS_create_point_url parameters:paraments progress:^(NSProgress * _Nonnull uploadProgress) {
                        ...
                        if(dic[@"status"])
                        {
                            ...
                            {
                                //发送数据出错
                                [EMAlertView showAlertWithTitle:@"提示" message:@"地理群组创建失败" completionBlock:^(NSUInteger buttonIndex, EMAlertView *alertView) {
                                    //返回
                        ...
                        //post 失败
                        [EMAlertView showAlertWithTitle:@"提示" message:@"群组创建失败" completionBlock:^(NSUInteger buttonIndex, EMAlertView *alertView) {
                            [weakSelf.navigationController popViewControllerAnimated:YES];
                        } cancelButtonTitle:@"确定" otherButtonTitles:nil];
